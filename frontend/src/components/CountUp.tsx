...
export default function CountUp({ value, decimals = 0, suffix = '', animate, delay = 0, className }: {
  ...
  const raf = useRef(0)
  useEffect(() => {
    if (!animate || reduced()) { setShown(value); return }
    const SCRAMBLE = 550, SETTLE = 950
    ...
    const tick = (now: number) => {
      ...
      if (t < SCRAMBLE) {
        ...
        const from = value < 50 ? value + 18 : value - 18
        setShown(from + (value - from) * ease + Math.sin(x * Math.PI) * (value < 50 ? -2.5 : 2.5) * (1 - x))
      } else {
        setShown(value)
        return
      }
